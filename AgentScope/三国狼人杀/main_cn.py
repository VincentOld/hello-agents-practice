...
# 读取 .env 里的 DeepSeek 配置（LLM_API_KEY / LLM_BASE_URL / LLM_MODEL_ID）
load_dotenv()

from agentscope.agent import ReActAgent
from agentscope.model import OpenAIChatModel  # DeepSeek 是 OpenAI 兼容接口，用这个
from agentscope.pipeline import MsgHub, sequential_pipeline, fanout_pipeline
from agentscope.formatter import DeepSeekMultiAgentFormatter  # AgentScope 自带的 DeepSeek formatter
    ...
    async def create_player(self, role: str, character: str) -> ReActAgent:
        ...
        agent = ReActAgent(
            ...
            model=OpenAIChatModel(
                model_name=os.environ.get("LLM_MODEL_ID", "deepseek-chat"),
                api_key=os.environ["LLM_API_KEY"],
                stream=False,  # 关掉流式：结构化输出一次性整段返回，消除“Error in block input”逐字解析报错
                # DeepSeek 的地址通过 client_args 传给底层 OpenAI 客户端
                client_args={"base_url": os.environ.get("LLM_BASE_URL", "https://api.deepseek.com/v1")},
            ),
...
async def main():
    ...
    if "LLM_API_KEY" not in os.environ:
        print("❌ 请在 .env 里设置 LLM_API_KEY（你的 DeepSeek key）")
        return
